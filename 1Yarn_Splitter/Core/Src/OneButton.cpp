                     ...
                     bool activeLow, bool pullupActive)
{
  ...
  // Use the common setup path so internal state (_pin, _buttonPressed)
  // and HAL GPIO configuration are consistently initialized.
  setup(port, pin, activeLow, pullupActive);
}  // OneButton


// initialize or re-initialize the input pin
void OneButton::setup(GPIO_TypeDef* port, uint16_t pin, bool activeLow, bool pullupActive, bool initHw)
{
    gpioPort = port;
    gpioPin  = pin;
    this->activeLow = activeLow;

    // mark pin as initialized so tick() will read the pin
    _pin = pin;

    // Determine logic level for button-pressed interpretation
    _buttonPressed = activeLow ? GPIO_PIN_RESET : GPIO_PIN_SET;

    // initialize debounce state from current pin level so there is no spurious event
    now = millis();
    bool currentLevel = (HAL_GPIO_ReadPin(port, pin) == _buttonPressed);
    debouncedLevel = currentLevel;
    _lastDebounceLevel = currentLevel;
    _lastDebounceTime = now;

    // Optionally reconfigure the GPIO. If initHw==false we preserve existing (e.g. EXTI) configuration.
    if (initHw) {
...
uint32_t OneButton::millis(void) {
  return HAL_GetTick();
}
